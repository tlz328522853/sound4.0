 ...
 */
@RestController
@RequestMapping(value = "/apihl/ljRecycler")
public class LjRecyclerController{
	...
	private Map<String,Object> resultData(Map<String,Object> dateMap,int recDay){
		...
		if(recDay==0){
			co =countService.countRecyleChartDate(startDate,endDate);
		}
		else{
			co = countService.countRecyleChartMonth(startDate,endDate);
		}
		//对比数据并补全
