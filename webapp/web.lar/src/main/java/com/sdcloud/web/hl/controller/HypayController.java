 ...
 */
@RestController
@RequestMapping(value = "/apihl/hypay")
...
//				int[] ydata_shui={100,200,300,400,500,600};//水
//				int[] ydata_dian={10,20,30,40,50,60};//电
//				int[] ydata_qi={1,2,3,4,5,6};//气
//				resultData.put("xdata", xdata);
//				resultData.put("ydata_shui", ydata_shui);
//				resultData.put("ydata_dian", ydata_dian);
//				resultData.put("ydata_qi", ydata_qi);
//			}
//			else{
//				resultData=new HashMap<>();
...
//	}
	...
	private Map<String,Object> resultData(Map<String,Object> dateMap,int recDay){
		...
		if(recDay==0){
			co =countService.countLifeChartDate(startDate,endDate);
		}
		else{
			co = countService.countLifeChartMonth(startDate,endDate);
		}
		//对比数据并补全
		double[] ydata_shui=new double[xdata.length];//水
		double[] ydata_dian=new double[xdata.length];//电
		double[] ydata_qi=new double[xdata.length];//气
		...
		for(int i=0;i<xdata.length;i++){
			if(maps.get(xdata[i])!=null){
				ydata_shui[i]=round(maps.get(xdata[i]).getNumDou1(),3);
				ydata_dian[i]=round(maps.get(xdata[i]).getNumDou2(),3);
				ydata_qi[i]=round(maps.get(xdata[i]).getNumDou3(),3);
				continue;
			}
			ydata_shui[i]=0;
			ydata_dian[i]=0;
