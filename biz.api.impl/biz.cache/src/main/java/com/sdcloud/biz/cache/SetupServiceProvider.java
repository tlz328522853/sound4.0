...
public class SetupServiceProvider {
	...
	private static final Logger logger = LoggerFactory.getLogger(SetupServiceProvider.class);
	...
	public static void main(String[] args) {
		...
		try {
			...
			if(args.length>0){
				runningInEclipse="true".equals(args[0])?true:false;
			}
			logger.info("Enter the :{} method ,running in eclipse is :{}", Thread.currentThread() .getStackTrace()[1].getMethodName(),runningInEclipse);
			...
			if (!isRunning()) {
				...
				logger.info("cache provider is setting up!! ");
			...
			e.printStackTrace();
			logger.error("cache provider is exiting!! ", e);
		}
	}
	...
	private static boolean isRunning() throws Exception {
		...
		try {
			...
			if (lock == null) {
				isRun = true;
				logger.info("cache service process.instance文件已被运行的实例锁定.退出启动!");
				System.exit(0);
			}

		} catch (Exception e) {
			throw new Exception("cache service执行isRunning方法时发生异常！", e);
		}
		return isRun;
	}
}
